...
module CodeSandboxMcp
  module Tools
    class ExecuteCode < Base
      ...
      class << self
        def call(language:, code:, session_id: nil, reset_session: false)
          ...
          # Execute in session
          result = session_manager.execute_in_session(session_id, language, code, executor)
          build_response(code, language, result, session_id)
        ...
        def build_response(code, language, result, session_id)
          content = [
            ...
            create_content_block(execution_metadata(result, session_id), final: true)
          ].compact
        ...
        def execution_metadata(result, session_id)
