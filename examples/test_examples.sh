#!/bin/bash
# Test script to verify examples work correctly with the Docker image
...
test_example() {
    ...
    if cat "$file" | docker run --rm -i code-sandbox:latest 2>/dev/null | grep -q '"isError":false'; then
...
for lang_code in "${LANGUAGES[@]}"; do
    ...
    if echo "$REQUEST" | docker run --rm -i code-sandbox:latest 2>/dev/null | grep -q "works"; then
        echo -e "${GREEN}✓ PASSED${NC}"
    else
        echo -e "${RED}✗ FAILED${NC}"
    fi
